...
const popularActorsUrl =
  "https://api.themoviedb.org/3/person/popular?language=en-US";
const latestNewsUrl =
  "https://api.themoviedb.org/3/movie/latest?language=en-US";
  ...
  try {
    ...
    if (data && data.results) {
      ...
      data.results.slice(0, 6).forEach((actor) => {
        popularActorsContainer.innerHTML += createCard(
          actor,
          "person",
          placeholderImage,
          false
    ...
    displayErrorMessage(
      "Failed to load popular actors. Please try again later."
    );
  }
}
