...
export async function handleSearch() {
  ...
  if (!query) {
    searchResultsContainer.innerHTML =
      '<p class="text-danger">No search query provided.</p>';
    return;
  }
  ...
  if (data && data.results && data.results.length > 0) {
    ...
    if (allMovies.length > 0) {
      ...
      allMovies.forEach((movie) => {
        searchResultsContainer.innerHTML += createCard(
          movie,
          "movie",
          placeholderImage,
          true
    ...
    // All TV Shows
    const allTVShows = data.results.filter(
      (result) => result.media_type === "tv"
    );
    if (allTVShows.length > 0) {
      searchResultsContainer.innerHTML +=
        '<h3 class="col-12">All TV Shows</h3>';
